...
\documentclass[
  ...
  biblatex={            % Options for biblatex (see biblatex documentation)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% uncomment for 'numeric-by-order-of-citation' like citations
    % jj  backend=bibtex,     % use bibtex if possible
    % jj  style=numeric-comp, % numeric(*), alphabetic, authoryear, bwl-FU
    % jj  sortcites=true,     % If numeric, sort cites by crescent order
    % jj  sorting=nyt,        % none, nyt(*), ynt
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% uncomment for 'numeric-by-order-of-citation' like citations
    % backend=bibtex,   % use bibtex if possible
...
\authorname[m]{Pedro Miguel Oliveira da Silva}{Pedro Silva}
...
% use [f] for female and [m] for male
\adviser[m]{José João Dias de Almeida}{Auxiliar Professor}{\\& Universidade do Minho} 
...
\addlisttofrontmatter{\input{Chapters/publication1.tex}}
